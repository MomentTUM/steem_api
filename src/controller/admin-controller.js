...
exports.getGameToData = async (req, res, next) => {
  ...
  try {
    ...
    if (!gameDetails) {
      return {
        message: `steam_appid:${appId} no data, the app doesn't add to the table`,
      };
    }
    const result = {
    ...
    const filterGame = gameDetails.hasOwnProperty("price_overview");
    const isFree = gameDetails.is_free;
    if (!filterGame && !isFree) {
      return {
        message: `steam_appid:${appId} price is null, the app doesn't add to the table`,
      };
    }
...
exports.getGamesToData = async (req, res, next) => {
  ...
  //   33230, 289650, 311560, 1151340, 588430, 377160, 22370, 38410, 38400, 413150,
  //   1222670, 47890, 550, 500, 1904540, 1488560, 872820, 324090, 1569040,
  //   1263850, 1100600, 872790, 482730, 1085660, 953690, 1861090, 497940, 782330,
  //   2300, 9050, 1148590, 379720, 2280, 239140, 534380, 278080, 322520, 730310,
  //   4000, 1593500, 1151640, 1817190, 1817070, 1659420, 1649240, 1259420, 394510,
  //   287700, 1552550, 393360, 601510, 311340, 1018010, 1018020, 996470, 770240,
  //   702700, 1665460,
  // ];
  //=========================================================================//
  //#1
